...
{
    public partial class Pay4 : Form
    {
        public Pay4()
        {
            InitializeComponent();
        ...
        private void Pay4_Load(object sender, EventArgs e)
        {
            //设置日期格式
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            String date = year + "-0" + month;                  //查询改所有员工当月的工资明细
            try
            {
                using (SqlConnection conn = new SqlConnection(SQLServer.connsql))
                {
                    ...
                    conn.Open(); // 打开数据库连接
                    String sql = "select sid,oday,aday,gold,tax,min,jiang,fa,shouldsalary,realsalary,time from mingxi where convert(varchar(7),mingxi.time,120) like '" + date + "'"; // 查询语句，包含date字段
                    ...
                    this.dataGridView1.Columns[4].HeaderText = "税率(%)";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            String year = dateTimePicker1.Value.Year.ToString();
            String month = dateTimePicker1.Value.Month.ToString();
            String date = year + "-0" + month;                  //查询改员工当月的工资明细
            int sid = int.Parse(textBox1.Text.Trim());
            try
            {
                using (SqlConnection conn = new SqlConnection(SQLServer.connsql))
                {
